...
if (isset($_POST['performance_report'])) {
    ...
    $progress_performance1  = mysqli_real_escape_string($conn, $_POST['progress_performance1']);
    $progress_performance2  = mysqli_real_escape_string($conn, $_POST['progress_performance2']);
    ...
    if (count($errors) == 0) {
        ...
        $sql = "INSERT INTO progress_info(project_id, progress_quarter, progress_performance1, progress_performance2,user_id) 
        VALUES ('$project_id','$progress_quarter','$progress_performance1','$progress_performance2','$user_id')";
        print_pre($sql);
        ...
        if ($_SESSION['success'] == "You are save project name") {
            ...
            foreach ($budgets as $key => $value) {
                $sql = "INSERT INTO project_budget(project_id, budget_group, item, price, quantity,progress_id) VALUES('{$value['project_id']}', '{$value['budget_group']}', '{$value['item']}', '{$value['price']}', '{$value['quantity']}', '{$value['progress_id']}')";
                print_pre($sql);
                mysqli_query($conn, $sql);
            }
            ...
            if ($loop) {
                ...
                if ($valueUpsert) {
                    ...
                    $sql = "INSERT INTO project_plant(project_id, plant_detail, plant_time,plant_place) VALUES $value";
                    print_pre($sql);
                    mysqli_query($conn, $sql);
                }
            }
